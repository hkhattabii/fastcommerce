const AUTH_SERVICE = {
    ...
    BYEMAIL: (value) => `http://167.172.41.60:30001/user?field=email&value=${value}`,
    BYFIELDANDVALUE: (field, value) => `http://167.172.41.60:30001?field=${field}&value=${value}`,
...
const CART_SERVICE = {
    ...
    INCREASE: (user_id, product_id) => `http://localhost:5007/increase?user_id=${user_id}&product_id=${product_id}`,
    DECREASE: (user_id, product_id) => `http://localhost:5007/decrease?user_id=${user_id}&product_id=${product_id}`,
    CLEAR: (user_id) => `http://localhost:5007/clear?user_id=${user_id}`,
    ROOT: (user_id, product_id) => {
        return user_id && product_id ? `http://localhost:5007?user_id=${user_id}&product_id=${product_id}` 
        : user_id ? `http://localhost:5007?user_id=${user_id}`
        : `http://localhost:5007`
    }
}
...
const DELIVERY_SERVICE = {
    ROOT: (user_id, delivery_id, bill_id) => {
        return delivery_id && bill_id ? `http://localhost:5015?delivery_id=${delivery_id}&bill_id=${bill_id}`
        : user_id ? `http://localhost:5015?user_id=${user_id}`
        : `http://localhost:5015`
    }
}
